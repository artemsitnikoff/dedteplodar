...
- Этап 3 — эскалация: таблица `b24_sessions` (`src/b24/models.py`, state-machine в `src/b24/sessions.py`),
  ...
  Триггеры в два слоя: регэкспы (кнопка, просьбы «соедините с оператором», «позвоните мне», фразы про личный
  заказ) и флаг `needs_human` из intent-экстрактора (Haiku); плюс 2 ошибки генерации подряд. Детали — §10 п. 9. После передачи бот молчит,
...
9. ✅ **Эскалация** → `imopenlines.bot.session.operator`, либо `.transfer` с `TRANSFER_ID = B24_TRANSFER_TO`
   (`queue<ID>` или id сотрудника, `LEAVE=N`). Триггеры в два слоя:
   - **слой 1, мгновенно, регэкспы** (`b24_service.is_operator_request` / `is_personal_order`): кнопка;
     «оператор/поддержка» + любое желание («хочу», «можно», «дайте»…); «менеджер/сотрудник/специалист/консультант/
     продавец» только с глаголом связи («соедините», «позовите», «свяжите», «поговорить с»…); «с человеком»,
     «живой человек», «дайте человека»; «позвоните мне», «с кем можно поговорить», «это бот»; одиночное
     «оператора!». Исключения: «оператор сказал…», «менеджер по опту», «сколько человек нужно…»; сообщения
     длиннее 80 символов проверяются только на «позвоните мне»/«с кем поговорить». Плюс фразы про личный заказ
     («мой заказ», «статус заказа», «не привезли», «брак», «возврат денег», «рекламация»).
   - **слой 2, через Haiku** — поле `needs_human` в intent-экстракторе (`src/rag/intent_extractor.py`, правило 9),
     пробрасывается как `AnswerMeta.needs_human`. Срабатывает после генерации (10–40 с): сгенерированный ответ
     НЕ отправляется, плейсхолдер редактируется в «Передаю диалог оператору». Ловит то, чего нет в списках:
     недовольство ботом, жалобы без ключевых слов. ⚠️ Промпт общий для Telegram/веб/Б24 — после деплоя нужен
     eval-прогон и сравнение с последним, чтобы классификация не поехала (там `needs_human` игнорируется).
   - `B24_MAX_CONSECUTIVE_ERRORS` ошибок генерации подряд. Клиенту уходит
