...
2. **faq_match_id**: Внимательно прочитай ВЕСЬ список FAQ ниже. Если есть запись, которая по смыслу отвечает на вопрос пользователя — верни её номер. Не нужна 100% текстовая идентичность — достаточно того, что **ответ из этой FAQ-записи был бы полезен пользователю**. Парафразы, сокращения, разговорные формулировки — всё это совпадение.
   ...
   **ИСКЛЮЧЕНИЕ 2 — подбор конкретной модели**: если пользователь хочет **конкретную рекомендацию товара / модели** («какую газовую печь порекомендуете?», «посоветуй модели на 18 м³», «что подойдёт под мою баню 12 кубов?», «подберите котёл на 100 кв.м») — это **RAG_PRODUCT**, FAQ-матч **НЕ применяется** (верни `null`). Даже если есть FAQ с **общей методикой подбора** (типа «как считать кубатуру»), он НЕ заменяет конкретный подбор по каталогу. Юзеру нужны названия моделей, а не формула.
