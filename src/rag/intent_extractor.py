...
2. **faq_match_id**: Внимательно прочитай ВЕСЬ список FAQ ниже. Если есть запись, которая по смыслу отвечает на вопрос пользователя — верни её номер. Не нужна 100% текстовая идентичность — достаточно того, что **ответ из этой FAQ-записи был бы полезен пользователю**. Парафразы, сокращения, разговорные формулировки — всё это совпадение.
   ...
   **ИСКЛЮЧЕНИЕ 2 — конкретный товар/модель**: если в запросе пользователя есть **конкретное название модели / серии** (Русь-12, Куппер ПРО 16, Метеор-150, Сахара-24, Сибирь-30, Былина и т.д.) ИЛИ пользователь просит конкретную рекомендацию («какую печь посоветуете?», «подберите котёл на 100 кв.м») — это **RAG_PRODUCT**, FAQ-матч **НЕ применяется** (верни `null`).
   FAQ-записи типа «Какую печь выбрать для парилки X?», «Есть ли модель X в наличии?», «Какой котёл нужен для дома X м²?» — это generic-плейсхолдеры с буквой X. Их матчить **только если** юзер спрашивает буквально про методику, без названия модели и без своих параметров. Когда упомянута конкретная модель/параметры — иди в RAG.
