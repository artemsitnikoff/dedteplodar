...
2. **faq_match_id**: Внимательно прочитай ВЕСЬ список FAQ ниже. Если есть запись, которая по смыслу отвечает на вопрос пользователя — верни её номер. Не нужна 100% текстовая идентичность — достаточно того, что **ответ из этой FAQ-записи был бы полезен пользователю**. Парафразы, сокращения, разговорные формулировки — всё это совпадение.
   **ВАЖНОЕ ИСКЛЮЧЕНИЕ — личные/конкретные вопросы**: если в вопросе есть «мой / моего / моей / для меня / тот что я заказал / у меня / в моём» — это вопрос про **конкретный заказ клиента**, а не про общие правила. FAQ-матч **НЕ применяется**, верни `null` — бот эскалирует на оператора через FAQ_COMPANY.
   Примеры (✅ матч):
   ...
   Примеры (❌ null):
   - User: «когда отгрузят МОЙ заказ?» / FAQ: «Какие сроки доставки до моего города?» → ❌ null. Личный кейс — эскалация на менеджера.
   - User: «где сейчас МОЯ посылка?» / FAQ есть про доставку → ❌ null. Статус конкретной посылки.
   - User: «Расскажи о Русь-12» / FAQ: «Расскажи о Былина-12» → ❌ null (разные модели).
   - User: «как лучше прогреть баню?» / FAQ нет такой темы → ❌ null.
   Принцип: ЩЕДРО матчь общие/обезличенные вопросы; СТРОГО отказывайся матчить личные вопросы с местоимениями «мой/моя/моего/у меня».
